<div id="posts-container" data-group-slug="{{ $group->slug ?? '' }}">
    ...
    <div class="p-5">
        ...
        <header class="flex items-start justify-between mb-4">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-medium">
                    {{ $post->user ? strtoupper(substr($post->user->username, 0, 1)) : '?' }}
                </div>
                <div>
                    <h3 class="font-medium">{{ $post->user ? $post->user->username : 'Deleted User' }}</h3>
                ...
                >
                    <div class="py-1" role="none">
                        ...
                        @if($canEdit)
                            ...
                            <a 
                                href="{{ route('forums.groups.posts.edit', ['group' => $group->slug, 'post' => $post->id]) }}" 
                                class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100"
                                role="menuitem"
                                tabindex="-1"
                                onclick="event.preventDefault(); document.getElementById('edit-post-{{ $post->id }}').submit();"
                            >
                                <i class="fas fa-edit mr-2"></i> Edit Post
                            </a>
                            <form id="edit-post-{{ $post->id }}" action="{{ route('forums.groups.posts.edit', ['group' => $group->slug, 'post' => $post->id]) }}" method="GET" class="hidden">
                                @csrf
                            </form>
                            
                            <!-- Delete option for post owner or group admin -->
                            <form action="{{ route('forums.groups.posts.destroy', ['group' => $group->slug, 'post' => $post->id]) }}" method="POST" class="w-full">
                                @csrf
                                @method('DELETE')
                                <button 
                                    type="button" 
                        ...
                        <button 
                            ...
                            role="menuitem"
                            tabindex="-1"
                            @click="navigator.clipboard.writeText('{{ url()->current() }}'); alert('Link copied to clipboard!'); open = false;"
                        >
        ...
        <div class="mb-4">
            ...
            <div class="mt-3 rounded-lg overflow-hidden">
                @if(in_array($post->media_type, ['image/jpeg', 'image/png', 'image/gif']))
                    <img src="{{ asset('storage/' . $post->media_path) }}" 
                         alt="Post image" 
                         class="w-full h-auto rounded-lg">
                @elseif($post->media_type === 'video/mp4')
        ...
        <div x-show="showComments" x-transition class="space-y-3 mt-4 pt-4 border-t border-gray-100 px-5 pb-5 -mx-5 -mb-5 bg-gray-50">
            ...
            <div class="flex items-start space-x-2">
                ...
                <div class="flex-1 bg-white rounded-lg p-3 border border-gray-100">
                    ...
                    @if($reply->media_path)
                        <div class="mt-2">
                            @if(str_starts_with($reply->media_type, 'image/'))
                                <img src="{{ asset('storage/' . $reply->media_path) }}" 
                                     alt="Reply image" 
                                     class="max-w-full h-auto rounded-lg border border-gray-200">
                            @elseif(str_starts_with($reply->media_type, 'video/'))
                                <video controls class="w-full rounded-lg border border-gray-200">
                                    <source src="{{ asset('storage/' . $reply->media_path) }}" type="{{ $reply->media_type }}">
                                    Your browser does not support the video tag.
                                </video>
                            @else
                                <a href="{{ asset('storage/' . $reply->media_path) }}" 
                                   class="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                    <i class="fas fa-file-download mr-2"></i>
                                    {{ $reply->media_name ?? 'Download File' }}
                                </a>
                            @endif
                        </div>
                    @endif
            ...
            <div class="pt-3 mt-3 border-t border-gray-100">
                <form class="reply-form" method="POST" enctype="multipart/form-data" 
                      ...
                      action="{{ route('forums.groups.posts.replies.store', ['group' => $group->slug ?? '', 'post' => $post->id]) }}">
                    @csrf
                    <div class="flex items-start space-x-2">
                        <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-xs font-medium flex-shrink-0">
                        ...
                        <div class="flex-1 relative">
                            <textarea name="content" 
                                    ...
                                    {{ !auth()->check() ? 'disabled placeholder="Please login to comment"' : '' }}></textarea>
                            <div class="absolute right-2 bottom-2 flex items-center space-x-1">
                                ...
                                <button type="submit" class="text-gray-400 hover:text-indigo-600 disabled:opacity-50" {{ !auth()->check() ? 'disabled' : '' }}>
...
<div class="mt-8 text-center">
    <button class="bg-indigo-50 text-indigo-600 px-6 py-2 rounded-full font-medium hover:bg-indigo-100 transition" id="load-more-posts" data-next-page="{{ $posts->nextPageUrl() }}">
