    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl p-1 mb-6 flex border-b border-gray-200">
            <a href="{{ route('forums.index') }}" 
               class="flex-1 py-3 px-4 text-center font-medium {{ request()->routeIs('forums.index') ? 'tab-active' : 'tab-inactive' }}">
                <i class="fas fa-users mr-2"></i>All Groups
            </a>
            @auth
            <div class="h-9 border-r border-gray-200 mx-auto w-px"></div>
            <a href="{{ route('forums.my-groups') }}" 
               class="flex-1 py-3 px-4 text-center font-medium {{ request()->routeIs('forums.my-groups') ? 'tab-active' : 'tab-inactive' }}">
            ...
            @forelse($forumGroups as $group)
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 border border-gray-100 relative">
                    ...
                    <div class="relative px-6 pb-5 -mt-8 bg-white rounded-t-xl z-10 py-4">
                        ...
                        <!-- Group Description -->
                        <div class="mt-2">
                            <h3 class="text-base font-semibold text-gray-900 mb-2 line-clamp-1">{{ $group->name }}</h3>
                        ...
                        <div class="flex items-center justify-between mb-5">
                            ...
                            <div class="text-right">
                                <div class="text-sm font-medium text-gray-700">{{ $group->posts_count }} {{ Str::plural('post', $group->posts_count) }}</div>
                                <div class="text-xs text-gray-500">{{ $group->members_count }} {{ Str::plural('member', $group->members_count) }}</div>
                            </div>
                        </div>
            
                        <!-- Action Button -->
                        <div class="pt-4">
                            @if(auth()->check() && ($group->isMember(auth()->user()) || $group->isAdmin(auth()->user())))
