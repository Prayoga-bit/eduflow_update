...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex items-center justify-between mb-3 px-1">
        ...
        <a href="{{ route('forums.index') }}" class="text-xs text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 px-2 py-1 rounded transition-colors">
            See all
        </a>
    </div>
    <div class="relative">
        <div class="flex space-x-3 overflow-x-auto pb-3 -mx-1 px-1" style="scrollbar-width: thin;">
            @if(auth()->check())
            <a href="{{ route('forum.groups.create') }}" class="flex-shrink-0 w-32 bg-white rounded-lg p-3 shadow-xs border border-gray-100 hover:shadow-sm transition-shadow">
            ...
            <a href="{{ route('forums.groups.show', $group->slug) }}" class="flex-shrink-0 w-32 bg-white rounded-lg p-3 shadow-xs border border-gray-100 hover:shadow-sm transition-shadow">
                <div class="flex items-center space-x-2">
                    <div class="w-6 h-6 rounded bg-gray-200 flex-shrink-0 overflow-hidden">
                        @if($group->avatar_path)
                        <img 
                            src="{{ asset('storage/' . $group->avatar_path) }}" 
                            alt="{{ $group->name }}" 
                    ...
                    <div class="min-w-0">
                        <h4 class="text-xs font-medium text-gray-900 truncate">{{ $group->name }}</h4>
                        <p class="text-xs text-gray-500 truncate">{{ $group->members_count }} members</p>
                    </div>
                </div>
            </a>
            @empty
...
<div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-6">
    ...
    @else
        <p class="text-sm text-gray-500">Please <a href="{{ route('login') }}" class="text-indigo-600 hover:text-indigo-800 font-medium">login</a> to create a post.</p>
    @endauth
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-medium">
                                @if($post->user)
                                    {{ strtoupper(substr($post->user->name ?? $post->user->name ?? 'U', 0, 1)) }}
                                @else
                                    U
                                @endif
                            </div>
                                ...
                                <div class="py-1" role="none">
                                    ...
                                    @if($isPostOwner)
                                        ...
                                        <a 
                                            ...
                                            role="menuitem"
                                            tabindex="-1"
                                            onclick="event.preventDefault(); document.getElementById('edit-post-form-{{ $post->postid }}').submit();"
                                        >
                                            <i class="fas fa-edit mr-2"></i> Edit Post
                                        </a>
                                        <form id="edit-post-form-{{ $post->postid }}" action="{{ route('posts.edit', $post->postid) }}" method="GET" class="hidden">
                                            @csrf
                                        </form>
                                        
                                        <!-- Delete option for post owner -->
                                        <form action="{{ route('posts.destroy', $post->postid) }}" method="POST" class="w-full">
                                            @csrf
                                            @method('DELETE')
                                            <button 
                                                type="button" 
                        ...
                        <div class="p-4">
                            <a href="{{ asset('storage/' . $post->media_path) }}" 
                               ...
                               class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                ...
                                @if(isset($extension))
                                    <span class="ml-2 text-xs text-gray-500">({{ strtoupper($extension) }})</span>
                                @endif
                            </a>
                        </div>
                    @endif
                        ...
                        <div class="flex items-center justify-center">
                            <div class="flex items-center space-x-2 border border-gray-200 rounded-full py-0.5 px-2">
                                ...
                                <button 
                                    ...
                                    <span class="like-count text-sm px-2">{{ $post->like_count ?? 0 }}</span>
                                </button>
