<div x-data="{
    ...
    async submitForm(event) {
        ...
        try {
            const response = await fetch('<?php echo e(route("tasks.store")); ?>', {
                method: 'POST',
                headers: {
            ...
            <div class="kanban-column bg-sky-50 rounded-lg shadow p-2 flex-shrink-0 flex flex-col" style="height: fit-content; max-height: calc(100vh - 200px);">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="bg-sky-200 text-sky-800 text-xs font-semibold px-3 py-1 rounded-full">
                        <?php echo e($tasks['todo']->count() ?? 0); ?>

                    </span>
                </div>
                <div class="space-y-3 flex-1 overflow-y-auto pr-1" id="todo-tasks">
                    <?php $__currentLoopData = ($tasks['todo'] ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="task-card bg-white border-l-4 border-sky-500 rounded-r-lg p-3 shadow-sm hover:shadow-md transition-all cursor-pointer h-32 flex flex-col group" 
                            ...
                            data-task='<?php echo json_encode($task, 15, 512) ?>'>
                            <div class="flex justify-between items-start mb-1">
                                <h4 class="font-medium text-gray-800 text-sm" title="<?php echo e($task->title); ?>"><?php echo e($task->title); ?></h4>
                                <button type="button" 
                                        onclick="event.stopPropagation(); if(confirm('Are you sure you want to delete this task?')) { deleteTask(<?php echo e($task->id); ?>); }" 
                                        class="text-gray-400 hover:text-red-500 flex-shrink-0 ml-2 opacity-0 group-hover:opacity-100 transition-opacity" 
                                        title="Delete task">
                                    <i class="fas fa-trash-alt text-xs"></i>
                                </button>
                            </div>
                            <?php if($task->description): ?>
                                <p class="text-xs text-gray-500 mb-2" title="<?php echo e($task->description); ?>"><?php echo e($task->description); ?></p>
                            <?php endif; ?>
            ...
            <div class="kanban-column bg-amber-50 rounded-lg shadow p-2 flex-shrink-0 flex flex-col" style="height: fit-content; max-height: calc(100vh - 200px);">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="bg-amber-200 text-amber-800 text-xs font-semibold px-3 py-1 rounded-full">
                        <?php echo e($tasks['in_progress']->count() ?? 0); ?>
            ...
            <div class="kanban-column bg-fuchsia-50 rounded-lg shadow p-2 flex-shrink-0 flex flex-col" style="height: fit-content; max-height: calc(100vh - 200px);">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="bg-fuchsia-200 text-fuchsia-800 text-xs font-semibold px-3 py-1 rounded-full">
                        <?php echo e($tasks['in_review']->count() ?? 0); ?>
            ...
            <div class="kanban-column bg-emerald-50 rounded-lg shadow p-2 flex-shrink-0 flex flex-col" style="height: fit-content; max-height: calc(100vh - 200px);">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="bg-emerald-200 text-emerald-800 text-xs font-semibold px-3 py-1 rounded-full">
                        <?php echo e($tasks['done']->count() ?? 0); ?>

                    </span>
                </div>
                <div class="space-y-3 flex-1 overflow-y-auto pr-1" id="done-tasks">
                    <?php $__currentLoopData = ($tasks['done'] ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div x-show="isOpen" 
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all" 
                ...
                <!-- Modal Body -->
                <form id="addTaskForm" action="<?php echo e(route('tasks.store')); ?>" method="POST" class="space-y-6 p-6">
                    <?php echo csrf_field(); ?>
                    <div>
    ...
    function setupAddTaskForm() {
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const userId = document.querySelector('meta[name="user-id"]')?.content || '<?php echo e(Auth::id()); ?>';
                if (userId) {
                ...
                try {
                    ...
                    if (response.status === 401 || (data && data.redirect)) {
                        window.location.href = data.redirect || '<?php echo e(route("login")); ?>';
                        return;
                    }
...
document.addEventListener('submit', async function(e) {
    if (e.target.matches('form[action="<?php echo e(route('tasks.store')); ?>"]')) {
        e.preventDefault();
...
<script>
    ...
    console.log('Kanban delete function loaded');
</script>
<?php $__env->stopPush(); ?><?php /**PATH C:\xampp\htdocs\eduflow\resources\views/tasks/partials/kanban.blade.php ENDPATH**/ ?>